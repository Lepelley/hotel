...
if (!isset($_GET['r']) && $_SERVER['REQUEST_URI'] != '/') {
    $_GET['r'] = trim($_SERVER['REQUEST_URI'], '/');
    if (($uri = stristr($_GET['r'], '?', true)) !== false) {
        $_GET['r'] = $uri;
    }
}
...
function my_dump($aData, $bExit = true)
{
...
$app['route'] = array_key_exists($app['route'], $config['mvc.routes']) ? $app['route'] : '_default'; // erreur 404
...
if ($app['view'] == 'admin') {
    $app['view-admin'] = $app['route-match']['view-admin']
        ? $app['route-match']['view-admin']
        : null;
    // Affichage direct de la vue (sans passer par le layout).
    render_view('admin/layout.phtml', $app);
} else {
